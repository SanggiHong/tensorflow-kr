...
* `tf.random_uniform_initializer(a, b)` a와 b를 포함하는 사이의 값들 사이([a, b])에서 균등한 분포의 임의값들로 초기화합니다,
...
코드를 하나의 convolution을 만드는 함수 `conv_relu`로 refactor 해봅시다:
...
왜냐하면, 변수를 재사용하는 scope를 설정할 때 해당 스코프가 무엇인지를 스트링으로만 명시해주었기 때문입니다.
